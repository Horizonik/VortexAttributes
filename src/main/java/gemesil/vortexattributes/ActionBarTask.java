...
public class ActionBarTask extends BukkitRunnable {
    ...
    public void run() {
        ...
        for (Player player : Bukkit.getOnlinePlayers()) {
            ...
            message = message.replace("%health%", StatsManager.getSkill(player, "health").toString());
            message = message.replace("%current_health%", Integer.toString((int) player.getHealth()));
            message = message.replace("%strength%", StatsManager.getSkill(player, "strength").toString());
            message = message.replace("%armor%", StatsManager.getSkill(player, "armor").toString());
            ...
            healthAttr.setBaseValue(Math.max(StatsManager.getSkill(player, "health") / 5, 0));

            // Set new strength of player
            strengthAttr = player.getAttribute(Attribute.GENERIC_ATTACK_DAMAGE);
            assert strengthAttr != null;
            strengthAttr.setBaseValue(Math.max(StatsManager.getSkill(player, "strength") / 10, 0));

            // Set new armor of player
            armorAttr = player.getAttribute(Attribute.GENERIC_ARMOR);
            assert armorAttr != null;
            armorAttr.setBaseValue(Math.max(StatsManager.getSkill(player, "armor"), 0));
        }
    }
}
